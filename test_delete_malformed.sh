...
"$SCRIPT" delete 1 >/tmp/test_delete_malformed.log
...
if grep -q "^1${DELIM}任务A${DELIM}pending$" "$DATA_FILE"; then
  echo "FAIL: task 1 should be deleted"
  exit 1
fi

if ! grep -q "^2${DELIM}任务B${DELIM}pending$" "$DATA_FILE"; then
  echo "FAIL: task 2 should be retained"
  exit 1
fi
